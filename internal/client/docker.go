...
// DockerReader provides an implementation of FileReader interface.
...
// NewDockerReader creates a new DockerReader.
func NewDockerReader(id string, client Docker) *DockerReader {
	return &DockerReader{id, client, []byte(nil), 0, 0}
}

// GetSize return the number of bytes read.
func (b *DockerReader) GetSize() int32 {
	return b.offset
}

// SetSize sets the size.
// DEPRECATED
func (b *DockerReader) SetSize(size int32) {
	// DEPRECATED
}

// HasNextChunk returns true if the size of data is greater than the offset.
func (b *DockerReader) HasNextChunk() bool {
	return b.offset < b.size
}

// Rewind set the offset to 0.
func (b *DockerReader) Rewind() {
	b.offset = 0
}
...
func (b *DockerReader) ReadNextChunk() ([]byte, error, error) {
	if b.size == b.offset {
		return []byte{}, nil, nil
	}
...
func (b *DockerReader) FetchSize() (int32, error, error) {
